# Phase 3 需求规格:麻将识别项目 — 28 类牌面分类器训练
...
四川麻将识别系统,两阶段架构。本 Phase 做第二阶段:**对检测器输出的牌面 crop 做 28 类分类**(w1~w9 / t1~t9 / b1~b9 / unknown,**无 back 类**)。

**最终验收目标(在真实 crop 冻结测试集上,见任务 2)**:
- 总体 top-1 ≥99%(剔除 unknown 后的 27 个牌面类)
...
2. 按 GT 的 28 类标签分文件夹输出 `frozen_crops_v1/`;每张 crop 文件名编码来源(图名+框id+尺寸桶+source 博主)
...
5. 检测置信度 0.5~0.65 的"可疑检出"单独放 `_review/` 文件夹(可能是误检,也是 unknown 类的好素材;若检出的其实是牌背,归入 unknown 或直接丢弃)
...
**train.py 功能需求**:
1. 模型:timm 加载 mobilenetv3_small_100 与 efficientnet_b0,输入 96×96,28 类输出;`--arch` 切换
...
**run_eval.py 功能需求**:
1. `--dataset dev|frozen|<path>`,底层调 Phase 0 eval_classification.py;输出总体/每类/尺寸分桶准确率、28×28 混淆矩阵、Top10 混淆对
...
4. 交付物目录:onnx + calibration.json + 输入预处理规格(resize 方式/归一化均值方差/颜色通道顺序,精确到数值)+ 28 类标签顺序表 + 对齐与速度报告
