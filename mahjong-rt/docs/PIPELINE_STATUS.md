...
区域判定已经实现在 `mahjong_rt/zones.py`,阈值来自 899 个人工标注框的网格搜索,实测整体
92.5%。**自己拍阈值几乎必然更差** —— 本项目前三版就是手调的,最好只有 39.7%,连"把所有牌
都判成牌池"这个笨基线(75.8%)都不如。失败原因在 `docs/tuning_findings.md`。

判定逻辑就是这 5 条,全部用**相对量**,不用像素:

```python
size_ratio = 框的短边 / 该帧所有框短边的中位数
nx, ny     = 框中心的归一化坐标 (0~1)

if size_ratio >= 1.4 and ny >= 0.60:            → my_hand      我的手牌
elif nx <= 0.28:                                 → seat_left    上家
elif nx >= 0.78:                                 → seat_right   下家
elif ny <= 0.28 and size_ratio <= 0.9:           → seat_across  对家
else:                                            → river        牌池
```
...
正确做法是**按 track 生命期对区域做多数投票**(状态机已实现):

```python
tile.zone_votes[zone] += 1
