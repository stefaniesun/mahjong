...
| 指标 | 实测 | 目标 | 状态 |
|---|---|---|---|
| 检查点类别准确率 | 96.67% | ≥99.5% | 未达标 |
| 检查点召回(仅可见 / 含遮挡) | 95.97% / 97.02% | — | — |
| 检查点精确率(仅可见) | 96.82% | — | — |
| **闪烁率(改判次数/牌)** | **0.0272** | <0.05 | **达标** |
| ID 切换率 | 24.9% | — | 偏高 |
| 确认延迟 | 未测(缺时间戳真值) | ≤0.5s | — |
| 吞吐 | 0.16 fps (CPU) | ≥30 fps | 硬件限制 |

**读"精确率"要小心**:检查点只标注**那一帧看得见**的牌,而流水线**故意保留被遮挡的牌**
(见 4.1 的 `visible` 字段)。合法保留的遮挡牌在这个口径下会被算成误报,所以
"含遮挡"的精确率(75.99%)**结构性偏低,不能当质量指标用**。要判断有没有幽灵牌,
用**数量不变量**:一副手牌只有 13~14 张。
...
曾经这三个数是 **62 / 28 / 52** —— 同一张牌被重复计了最多 4 次。根因是
`lost_after` 调长(为了不让被遮挡的牌闪没)之后,断裂的 track 以新 ID 回来时
**旧条目没被清掉**。修法见 `state` 段的 `merge_dist` / `merge_iou` / `hand_lost_after`。
